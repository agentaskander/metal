...
const guidePages = [
  ...
  },
  {
    ...
    takeaways: ['Manufacturing credibility requires process clarity.', 'Panel orders should connect product, trim, finish, packaging, and logistics.', 'Better quote inputs reduce avoidable project friction.'],
...
const topicalClusters = [
  {
    ...
    slug: 'metal-panel-manufacturing',
  ...
  {
    name: 'Materials and Finish Guides',
  ...
  {
    name: 'Trim, Flashing, and Quote Guides',
    ...
    slug: 'trim-flashing-quote-readiness',
...
const articleReviewedDate = '2026-06-20'
...
const guideEnhancements: Record<string, {
  ...
  'metal-panel-manufacturing-guide': {
    ...
    comparison: [
      ['Manufacturing layer', 'Weak signal', 'Stronger buyer signal'],
    ...
    faq: [
      ...
      ['Why does Americas Panel Fab publish manufacturing education if American Super Panel sells panels?', 'Americas Panel Fab provides the company and manufacturing context. American Super Panel gives contractors and buyers a direct product path for exposed-fastener panel quotes.'],
    ...
    deepDive: [
      ...
      ['Why the manufacturer and panel system both matter', 'Americas Panel Fab gives buyers the company and manufacturing background behind the panel system. American Super Panel gives contractors a direct place to review exposed-fastener roofing and siding panel information.'],
    ],
    checklist: [
  ...
  'metal-panel-profiles-guide': {
    paragraphs: [
      ...
      'Americas Panel Fab publishes profile education for buyers who are still comparing systems and need a manufacturer-level explanation of profile families and decision factors. Buyers who know they want exposed-fastener roofing or siding can continue to American Super Panel for product-specific details.',
      'The practical value is answering the hidden questions behind the search. A search for R-Panel may really mean: What is the coverage width? Can it be used for siding? What trim is needed? What gauge should I ask for? What finish path makes sense? How do I compare quotes? Americas Panel Fab answers those questions without overclaiming project-specific performance.',
    ],
    ...
    deepDive: [
      ...
      ['Profiles connect research to real buying intent', 'A visitor researching profiles may not be ready to quote today, but they are showing commercial intent. If the page teaches them how to compare systems and then gives a clean path to American Super Panel when ready, the education supports revenue without sounding like a thin sales page.'],
  ...
  'pbr-panel-vs-r-panel': {
    paragraphs: [
      ...
      'This guide helps contractors, owners, and buyers ask better questions before they send plans or approve a material package. American Super Panel provides product-specific exposed-fastener panel information, while Americas Panel Fab explains why drawings, trim scope, finish path, and project conditions matter.',
    ],
  ...
  'commercial-metal-roofing-guide': {
    paragraphs: [
      ...
      'Americas Panel Fab connects manufacturing credibility with buyer readiness. Company-level resources help owners, contractors, and partners understand the decision process. Buyers ready for exposed-fastener panels can continue to American Super Panel for panels, siding, trim, flashing, and plan review.',
    ],
    ...
    faq: [
      ...
      ['Why does Americas Panel Fab link to American Super Panel?', 'Americas Panel Fab provides company and manufacturing context. American Super Panel gives buyers a direct path for exposed-fastener roofing, siding, trim, and quote requests.'],
    ],
    fieldNote: 'Commercial metal roofing buyers usually need both education and action. Americas Panel Fab answers research and manufacturing questions; American Super Panel supports product-ready quote requests.',
...
function articleSchema(guide: (typeof guidePages)[number]) {
  ...
  const article = {
    ...
    dateModified: articleReviewedDate,
    datePublished: articleReviewedDate,
  ...
  const breadcrumbs = breadcrumbSchema([
    ['Home', '/'],
    ['Guides', '/guides'],
    ...(cluster ? [[cluster.name, `/guides/${cluster.slug}`] as [string, string]] : []),
...
function getSeoConfig(path: string, statePage?: (typeof states)[number]): SeoConfig {
  ...
  const clusterSlug = path.startsWith('guides/') ? path.replace('guides/', '') : ''
  const clusterPage = topicalClusters.find((cluster) => cluster.slug === clusterSlug)

  if (clusterPage) {
    return {
      canonicalPath: `/guides/${clusterPage.slug}`,
      description: `${clusterPage.name} from Americas Panel Fab: ${clusterPage.copy}`,
      title: `${clusterPage.name} | Americas Panel Fab Guides`,
      schema: [
        {
          '@context': 'https://schema.org',
          '@type': 'CollectionPage',
          name: clusterPage.name,
          url: `${apfOrigin}/guides/${clusterPage.slug}`,
          description: clusterPage.copy,
        },
        breadcrumbSchema([
          ['Home', '/'],
          ['Guides', '/guides'],
          [clusterPage.name, `/guides/${clusterPage.slug}`],
        ]),
      ],
    }
  }
  ...
  const routeSeo: Record<string, SeoConfig> = {
    ...
    'custom-fabrication': {
      ...
      description:
        'Custom sheet metal fabrication guidance for trim, flashing, edge metal, project details, and contractor manufacturing partner support.',
...
function RoutedPage({ path }: { path: string }) {
  ...
  if (clusterPage) {
    return <ClusterHubPage cluster={clusterPage} />
  }
    ...
    <>
      <section className="relative overflow-hidden border-b border-slate-200 bg-white">
        ...
        <div className="section relative">
          <div className="max-w-4xl">
            <p className="eyebrow text-[#f97316]">Company • Factory • Manufacturing</p>
        ...
        <div className="grid gap-10 lg:grid-cols-[0.85fr_1.15fr] lg:items-start">
          <div>
            <SectionIntro
              ...
              title="A clear manufacturer behind a focused panel system."
              copy="Americas Panel Fab manufactures and supports American Super Panel™ with company-level capability, fabrication knowledge, commercial documentation, and partner coordination."
            />
            <p className="mt-6 rounded border border-slate-200 bg-slate-50 p-4 text-sm font-bold leading-6 text-slate-700">
              Relationship note: American Super Panel™ is manufactured by Americas Panel Fab.
              Industry resources on this site may also reference suppliers, installers, or partners
              when they are relevant to metal panel projects.
            </p>
          </div>
          <div className="grid gap-4 sm:grid-cols-2">
            {[
              ['American Super Panel™', 'Flagship exposed-fastener roofing and siding panel system manufactured by Americas Panel Fab.'],
              ['Americas Panel Fab', 'Manufacturing company focused on fabrication, roll forming, panel systems, documentation, and commercial readiness.'],
              ['Project Resources', 'Guides and resources help buyers prepare drawings, finish questions, trim scope, and product comparisons before review.'],
              ['Partner Coordination', 'Supplier, installer, dealer, and manufacturing relationships can be reviewed where they support a specific project need.'],
            ].map(([title, copy]) => (
      ...
      <section className="relative overflow-hidden border-b border-slate-200 bg-white">
        ...
        <div className="section relative">
          <div className="max-w-4xl">
            ...
            <p className="mt-6 text-xl leading-8 text-slate-700">
              American Super Panel is the exposed-fastener roofing and siding panel
              product family manufactured by Americas Panel Fab for contractor, commercial,
              agricultural, and industrial building projects.
            </p>
            <div className="mt-8 flex flex-wrap gap-3">
        ...
        <div className="grid gap-10 lg:grid-cols-[0.82fr_1.18fr] lg:items-start">
          <div>
            ...
            title="A focused product family supported by the manufacturing company behind it."
              copy="Americas Panel Fab provides the manufacturing background, fabrication resources, and company-level support behind American Super Panel roofing and siding products."
            />
      ...
      <section className="section bg-white">
        <div className="grid gap-8 lg:grid-cols-[0.8fr_1.2fr] lg:items-center">
          <SectionIntro
            ...
            title="Ready for product details or a quote?"
            copy="Use American Super Panel for product-specific quote requests, plan uploads, dealer conversations, color questions, and order details."
          />
...
function Breadcrumbs({ items }: { items: [string, string][] }) {
  return (
    <nav aria-label="Breadcrumb" className="border-b border-slate-200 bg-slate-50 px-5 py-3 text-sm font-bold text-slate-600 lg:px-8">
        ...
        {items.map(([label, href], index) => (
          <li key={`${href}-${label}`} className="flex items-center gap-2">
            ...
            {index === items.length - 1 ? (
              <span className="text-[#0b1f33]">{label}</span>
            ) : (
              <a href={href} className="hover:text-[#f97316]">{label}</a>
            )}
          </li>
        ))}
      </ol>
...
function ClusterHubPage({ cluster }: { cluster: (typeof topicalClusters)[number] }) {
  const clusterGuides = cluster.slugs
    .map((slug) => guidePages.find((guide) => guide.slug === slug))
    .filter(Boolean) as (typeof guidePages)[number][]

  return (
    <>
      <Breadcrumbs items={[['Home', '/'], ['Guides', '/guides'], [cluster.name, `/guides/${cluster.slug}`]]} />
      <PageHero title={cluster.name} copy={cluster.copy} />
      <section className="section bg-white">
        <div className="grid gap-8 lg:grid-cols-[0.75fr_1.25fr]">
          <aside className="rounded border border-slate-200 bg-slate-50 p-6">
            <p className="eyebrow text-[#f97316]">Guide Path</p>
            <h2 className="mt-3 text-3xl font-black text-[#0b1f33]">Start here, then go deeper.</h2>
            <p className="mt-4 leading-7 text-slate-600">
              This guide path groups related topics so buyers, contractors, suppliers, and partners can move from broad research to a more specific project question.
            </p>
            <a className="mt-6 inline-flex items-center font-black text-[#0b1f33] hover:text-[#f97316]" href="/guides">
              View all guides <ArrowRight className="ml-2" size={18} />
            </a>
          </aside>
      ...
      <section className="section bg-slate-50">
        <SectionIntro
          eyebrow="How To Use This Path"
          title="Build a better project conversation before requesting pricing."
          copy="Use the guides in this path to clarify profile choices, material assumptions, fabrication details, documentation needs, and the information a manufacturer needs before quoting or production review."
        />
      </section>
    </>
  )
...
function GuidesPage() {
  return (
    <>
      <Breadcrumbs items={[['Home', '/'], ['Guides', '/guides']]} />
      <PageHero
      ...
      <section className="section bg-white">
        <SectionIntro
          eyebrow="Guide Paths"
          title="Metal panel resources organized by real project decisions."
          copy="Start with the question you are trying to answer: manufacturing process, panel profile selection, material and finish choices, trim details, or quote preparation."
        />
        <div className="mt-10 grid gap-6 lg:grid-cols-2">
          {topicalClusters.map((cluster) => (
            <article key={cluster.name} className="rounded border border-slate-200 bg-slate-50 p-6 shadow-sm">
              ...
              <a href={`/guides/${cluster.slug}`} className="mt-5 inline-flex items-center font-black text-[#0b1f33] hover:text-[#f97316]">
                Open guide path <ArrowRight className="ml-2" size={18} />
              </a>
      ...
      <section className="section bg-slate-50">
        <SectionIntro
          ...
          title={`${guidePages.length} metal panel guides for buyers, contractors, and partners.`}
          copy="Each guide answers a practical project-planning question and connects back into related Americas Panel Fab manufacturing guides and company resources."
        />
...
function GuideArticle({ guide }: { guide: (typeof guidePages)[number] }) {
  ...
  const cluster = topicalClusters.find((item) => item.slugs.includes(guide.slug))
  const tocItems = [
    ...(enhancement ? [['Pillar Overview', '#pillar-overview'], ['Project Evidence', '#project-evidence'], ['Deep Dive', '#deep-dive']] as [string, string][] : []),
    ...guide.sections.map(([title]) => [title, `#${slugifyTerm(title)}`] as [string, string]),
    ...(enhancement ? [['Decision Flow', '#decision-flow'], ['Comparison Table', '#comparison-table'], ['Quote Readiness Checklist', '#quote-readiness'], ['FAQ', '#faq']] as [string, string][] : []),
  ]
      ...
      <section className="section bg-white">
        <div className="grid gap-10 lg:grid-cols-[0.88fr_1.12fr] lg:items-start">
          <div>
            ...
            {tocItems.length ? (
              <nav aria-label="Table of contents" className="mt-6 rounded border border-slate-200 bg-slate-50 p-6 lg:sticky lg:top-28">
                <p className="eyebrow text-[#f97316]">On This Page</p>
                <div className="mt-4 grid gap-2">
                  {tocItems.map(([label, href]) => (
                    <a key={href} href={href} className="rounded border border-slate-200 bg-white px-4 py-3 text-sm font-black text-[#0b1f33] hover:border-[#f97316] hover:text-[#f97316]">
                      {label}
                    </a>
                  ))}
                </div>
          ...
          <article className="grid gap-5">
            {enhancement ? (
              <section id="pillar-overview" className="scroll-mt-28 rounded border border-slate-200 bg-white p-6 shadow-lg">
            ...
            {enhancement?.deepDive?.length ? (
              <section id="deep-dive" className="scroll-mt-28 rounded border border-slate-200 bg-white p-6 shadow-lg">
            ...
            {guide.sections.map(([title, copy]) => (
              <section id={slugifyTerm(title)} key={title} className="scroll-mt-28 rounded border border-slate-200 bg-white p-6 shadow-lg">
            ...
            {enhancement ? (
              <>
                ...
                {enhancement.checklist?.length ? (
                  <section id="quote-readiness" className="scroll-mt-28 rounded border border-slate-200 bg-white p-6 shadow-lg">
                ...
                <section id="faq" className="scroll-mt-28 rounded border border-slate-200 bg-white p-6 shadow-lg">
    ...
    <>
      ...
      <section className="section bg-slate-50">
        <SectionIntro
          ...
          copy="Short definitions are useful, but the real value comes from connecting each term to profiles, materials, fabrication, submittals, and commercial buying decisions."
        />
        <div className="mt-10 grid gap-5 md:grid-cols-2 lg:grid-cols-3">
          ...
          <div className="mt-8 rounded border border-slate-200 bg-white p-5 shadow-sm">
            ...
            <div className="mt-5 grid gap-3 sm:grid-cols-2">
              {['California', 'Arizona', 'Texas', 'Florida'].map((state) => (
                <a key={state} href={`/${state.toLowerCase()}-metal-panels`} className="rounded border border-slate-200 bg-slate-50 p-4 font-black text-[#0b1f33] hover:border-[#f97316] hover:text-[#f97316]">
                  {state} metal panel resources
                </a>
              ))}
            </div>
          </div>
